...
def sample_valid_coords_with_frequencies(num_points, gt_labels, 
    ...
    point_sampling_weights = sample_weights[gt_labels].view(-1, 1, 1, 1) * gt_masks # gtN x X x Y x Z
    ...
    point_indices = torch.multinomial(point_sampling_weights, 
                        num_samples=num_points, replacement=False) # coords in flatten voxel space
                        
    point_coords = unravel_indices(point_indices, shape=gt_masks.shape[1:]).float() # coords in voxel space
